...
function detectsDocumentAction(snippet) {
    return /\b(generate|create|make|prepare|build|export|turn)\b/i.test(snippet) &&
        /\b(pdf|docx|word|document|report|brief|attachment|file)\b/i.test(snippet);
}

function detectsDriveReadAction(snippet) {
    return /\b(google drive|drive)\b/i.test(snippet) &&
        /\b(search|find|list|read|open|look|download|get|show|retrieve|load|fetch|see|check)\b/i.test(snippet);
}

function detectsDriveWriteAction(snippet) {
    const s = String(snippet || '').toLowerCase();
    if (/\b(upload|save|move|delete|rename|send|export|push)\b.*\bdrive\b/.test(s)) return true;
    if (/\bcreate\b.*\bfolder\b/.test(s)) return true;
    if (/\bto\s+drive\b/.test(s)) return true;
    return false;
}
...
function detectsGithubAction(snippet) {
    ...
    if (/\b(worked on|recent change|recent changes|latest work|recent activity|what changed|what's new|history|commit log)\b/.test(s)) return true;
    return false;
}
